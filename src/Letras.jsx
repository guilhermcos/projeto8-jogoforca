export default function Letras(props) {
    ...
    function letras(word) {
        if (props.seGanhou === "sim" || props.seGanhou === "não") {
            return <button key={word} data-test="letter" onClick={() => props.compara(word)} className="letra-desabilitada" disabled={true}><p>{word.toUpperCase()}</p></button>;
            ...
            return <button key={word} data-test="letter" onClick={() => props.compara(word)} className={classe} disabled={disabled} ><p>{word.toUpperCase()}</p></button>;
        } else {
            return <button key={word} data-test="letter" onClick={() => props.compara(word)} className="letra-desabilitada" disabled={true} ><p>{word.toUpperCase()}</p></button>;
        }
    }
